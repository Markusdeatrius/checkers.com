...
import { validateMove, applyMove, Move } from "../utils/board";
...
export const setupGameSocket = (io: Server) => {
  io.on("connection", (socket) => {
    ...
    socket.on("makeMove", async (payload: any) => {
      ...
      const game = await prisma.game.findUnique({ where: { id: gameId } });
      if (!game) return socket.emit("error", "Game not found");
      if (game.turn !== userId) return socket.emit("error", "Not your turn");

      let board = game.board as number[][];
      const piece = board[from[0]][from[1]];
      const isKing = Math.abs(piece) > 2; // >2 označuje královnu

      const { valid, captured, mustJump } = validateMove(board, { from: from as [number, number], to: to as [number, number], playerId: userId, isKing }, userId);

      if (!valid) return socket.emit("error", "Invalid move");

      // Provedení tahu
      board = applyMove(board, from as [number, number], to as [number, number], captured);
